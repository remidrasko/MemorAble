...
class Questionnaire {
    ...
    public static function insererQuestionnaireAvecImage($dbh, $nom, $auteur, $type, $description, $ouverture, $adresseimage) {
        $sth = $dbh->prepare("INSERT INTO `questionnaires`(nom, auteur, type, ouverture, realisations, nbquestions, description, adresseimage) VALUES(?, ?, ?, ?, 0, 0, ?, ?)");
        $sth->execute(array($nom, $auteur, $type, $ouverture, $description, $adresseimage));
    }

    public static function insererQuestionnaireSansImage($dbh, $nom, $auteur, $type, $description, $ouverture) {
    ...
    public static function promouvoir($dbh, $id) {
        $type = Questionnaire::getQuestionnaireById($dbh, $id)->type;
        if ($type == 0) {
            $sth = $dbh->prepare("UPDATE `questionnaires` SET pageaccueil = 0 WHERE pageaccueil = 1 AND type = 0");
            $sth->execute();
            $sth = $dbh->prepare("UPDATE `questionnaires` SET pageaccueil = 1 WHERE id=?");
            $sth->execute(array($id));
        } else if ($type == 2) {
            $sth = $dbh->prepare("UPDATE `questionnaires` SET pageaccueil = 0 WHERE pageaccueil = 1 AND type = 2");
            $sth->execute();
            $sth = $dbh->prepare("UPDATE `questionnaires` SET pageaccueil = 1 WHERE id=?");
            $sth->execute(array($id));
        }
    ...
    public static function promus($dbh) {
        $tab = array();
        $query = "SELECT * FROM `questionnaires` WHERE `pageaccueil` = 1 ORDER BY type";
        $sth = $dbh->prepare($query);
        $sth->setFetchMode(PDO::FETCH_CLASS, 'Questionnaire');
        $sth->execute(array());
        while ($questionnaire = $sth->fetch()) {
    ...
    public static function supprimer($dbh, $id) {
        $sth = $dbh->prepare("DELETE FROM `questionnaires` WHERE id = ? ");
        $sth->execute(array($id));
    }
    ...
    static function getQuestionnaireProduit($dbh, $login, $idquestionnaire, $type) {
        ...
        if (isset($questionnaireproduit) && $questionnaireproduit != null) {
            ...
            if ($type == 0) {
                ...
                $tab = Question_Image::getQuestions($dbh, $idquestionnaire);
                foreach ($tab as $question) {
                    $sth = $dbh->prepare("INSERT INTO `utilisateur_question`(idquestion, login, nbtentatives, nbsucces, derniere_reponse, idquestionnaire) VALUES(?, ?, 0, 0, false, ?)");
                    $sth->execute(array($question->idquestion, $queprod->login, $queprod->idquestionnaire));
                }
...
class Remplissage {
    ...
    public function tableauRemplissages($dbh, $idproduit) {
        $query = "SELECT * FROM remplissages WHERE idproduit = ?";
        $sth = $dbh->prepare($query);
        $sth->setFetchMode(PDO::FETCH_CLASS, 'Remplissage');
        $sth->execute(array("$idproduit"));
        while ($remplissage = $sth->fetch()) {
            $tableauremplissages[$remplissage->date] = $remplissage->pourcentage;
        }
        if (isset($tableauremplissages)) {
            return $tableauremplissages;
        }
        return null;
    }
...
class Question_Image {
    ...
    public static function insererQuestion($dbh, $idquestionnaire, $reponse, $adresseimage, $abscisse, $ordonee, $numeroquestion) {
        $sth = $dbh->prepare("INSERT INTO `question_image`(idquestionnaire, reponse, adresseimage, abscisse, ordonnee, numeroquestion) VALUES(?, ?, ?, ?, ?, ?)");
        $sth->execute(array($idquestionnaire, $reponse, $adresseimage, $abscisse, $ordonee, $numeroquestion));
        $sth = $dbh->prepare("UPDATE `questionnaires` SET nbquestions = nbquestions +1 WHERE id = ?");
        $sth->execute(array($idquestionnaire));
    }
    ...
    public static function getQuestions($dbh, $idquestionnaire) {
        $tab = array();
        $query = "SELECT * FROM `question_image` WHERE `idquestionnaire` = ? ORDER BY numeroquestion";
        $sth = $dbh->prepare($query);
        $sth->setFetchMode(PDO::FETCH_CLASS, 'Question');
        $sth->execute(array("$idquestionnaire"));
        while ($questionnaire = $sth->fetch()) {
    ...
    public static function getQuestion($dbh, $idquestion) {
        $tab = array();
        $query = "SELECT * FROM `question_image` WHERE `idquestion` = ? ";
        $sth = $dbh->prepare($query);
        $sth->setFetchMode(PDO::FETCH_CLASS, 'Question_Image');
        $sth->execute(array("$idquestion"));
        $question = $sth->fetch();
        $sth->closeCursor();
        if (isset($question)) {
...
class Resultat_Evaluation {

    public $idquestionnaire;
    public $pourcentage;

    public static function insererPourcentage($dbh, $id, $pourcentage) {
        $sth = $dbh->prepare("INSERT INTO `resultat_evaluation`(idquestionnaire, pourcentage) VALUES(?, ?)");
        $sth->execute(array($id, $pourcentage));
    }

    public static function comparaison($dbh, $id, $score) {
        $query = "SELECT COUNT(*) FROM `resultat_evaluation` WHERE `idquestionnaire`= ? AND `pourcentage` <= ?";
        $sth = $dbh->prepare($query);
        $sth->execute(array("$id", "$score"));
        $a = $sth->fetch()["COUNT(*)"];
        $query = "SELECT COUNT(*) FROM `resultat_evaluation` WHERE `idquestionnaire`= ?";
        $sth = $dbh->prepare($query);
        $sth->execute(array("$id"));
        $b = $sth->fetch()["COUNT(*)"];
        if ($b == 0) {
